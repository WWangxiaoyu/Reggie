...
//        redisTemplate.delete(keys);
    ...
    public R<Page> page(int page, int pageSize, String name){
        ...
        //对象拷贝  参数为原对象，目标对象，可忽略的不拷贝的参数(将dish对象都拷贝到dishDto对象中，但是不拷贝records参数)
        ...
        List<DishDto> list = records.stream().map((item) -> {
            ...
            //由于菜品和分类数据是导入到数据库，不是输入后存到数据库，所以有可能会有查找不到分类的情况
            if(category != null){
...
//    }
    ...
    public R<List<DishDto>> list(Dish dish){
        ...
        //动态构造key
        String key = "dish_" + dish.getCategoryId() + "_" + dish.getStatus();//dish_1620629568998014978_1
        ...
        //如果不存在，需要查询数据库，将查询到的菜品数据缓存在redis中
